  ...
  try {
    ...
    for (const [batchIndex, batch] of batches.entries()) {
      ...
      try {
        ...
        console.log(`✅ Batch ${batchIndex + 1} completed. Progress: ${trackingData.statistics.successCount + trackingData.statistics.failureCount}/${allContent.length} (${trackingData.statistics.successCount} successful, ${trackingData.statistics.failureCount} failed)`);
    ...
    const response = {
      ...
      details: {
        ...
        batchesProcessed: finalTrackingData.statistics.batchesProcessed,
        averageTimePerActivity: finalTrackingData.summary.statistics.averageTimePerActivity
      }
    };

    // Include sample of failed entries if there are any (limit to first 10 for response size)
    if (finalTrackingData.failedMigrations.length > 0) {
      response.sampleFailedEntries = finalTrackingData.failedMigrations.slice(0, 10).map(failure => ({
      ...
      if (finalTrackingData.failedMigrations.length > 10) {
        response.note = `Showing first 10 of ${finalTrackingData.failedMigrations.length} failed entries. Check log files for complete details.`;
      }
    }
